...
if (!length(proteomes_paths)) stopmsg("No proteome FASTA files found in NCBI_Proteomes after filtering.")
...
# read_domtbl_simple extracts the domain-level i-Evalue and bitscore (i-score)
# Column indices interpret HMMER3 domtblout order where i-Evalue is at column 13 and i-score (bits) is at column 14.
...
# Filter domtbl rows to require thresholds:
...
if (!length(anchors)) stopmsg("Anchor FASTA seems empty")
ids <- names(anchors)
...
for (fam in families) {
  ...
  for (fa in cand_fastas) {
    ...
    # Keep only domtbl rows with i-Evalue <= 1e-5 AND bitscore >= 10
